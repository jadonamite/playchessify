...
/* ── PRELOADS ── Only the King is shown above the fold (landing hero). The other pieces load on demand where they are actually rendered (e.g. the game board), so we don't saturate the hero's bandwidth fetching ~6MB of unused GLBs. */
...
interface PieceProps {
  ...
  /* Optional PBR overrides so a caller can dial in a more physical, reflective material (e.g. the hero king) without affecting the default matte pieces. */
  roughness?: number;
  metalness?: number;
}
...
export function PieceView({ type, color, className = "w-12 h-12" }: { type: 'king' | 'queen' | 'rook' | 'pawn' | 'bishop' | 'knight', color?: string, className?: string }) {
  const { modelPath, scale } = pieceConfig[type];
    ...
    <div className={className}>
      <Canvas camera={{ position: [0, 0, 4], fov: 45 }} gl={{ alpha: true }}>
        ...
        <BasePiece
          ...
          floatSpeed={2}
          floatIntensity={0.5}
          position={type === 'king' || type === 'queen' ? [0, -1, 0] : [0, -0.8, 0]}
        />
      </Canvas>
    </div>
  );

...
const renderProgressBar = (isInfinite: boolean, progress?: number) => (
  <div className="w-full h-[2px] bg-white/5 relative overflow-hidden rounded-full border border-white/10">
    <motion.div
      ...
      initial={{ width: 0 }}
      animate={getProgressBarAnimation(isInfinite, progress)}
      transition={getProgressBarTransition(isInfinite)}
    />
  </div>
);

const renderStatusText = (message: string, progress?: number) => (
  ...
  return (
    <div className="flex flex-col items-center justify-center w-full py-24 gap-12 relative overflow-hidden">
      ...
      <div className="w-full max-w-sm flex flex-col items-center gap-4">
        {renderProgressBar(isInfinite, progress)}
        {renderStatusText(message, progress)}
      </div>

...
function FloatingPieces() {
  ...
  const cyanMaterial = useMemo(() => new THREE.MeshStandardMaterial({ color: '#00ccff', emissive: '#00ccff', emissiveIntensity: 0.4, roughness: 0.2, metalness: 0.8 }), []);
  const slateMaterial = useMemo(() => new THREE.MeshStandardMaterial({ color: '#0f172a', roughness: 0.4, metalness: 0.6 }), []);
...
export default function ComingSoonOverlay({ isOpen, onClose }: ComingSoonOverlayProps) {
  ...
  return (
    <AnimatePresence>
      {isOpen && (
        <motion.div initial={{ opacity: 0 }} animate={styles.overlay} exit={{ opacity: 0 }} className="m-sheet-wrap fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 box-border" style={{ background: 'rgba(5, 5, 15, 0.85)', backdropFilter: 'blur(12px)' }}>
          ...
          <div className="m-sheet relative z-10 flex flex-col items-center justify-center text-center w-full max-w-3xl box-border">
            <motion.div initial={{ opacity: 0, y: 30, scale: 0.95 }} animate={styles.content} transition={{ delay: 0.2, type: 'spring', stiffness: 200, damping: 20 }} className="flex flex-col items-center gap-5 md:gap-6 p-6 sm:p-8 md:p-16 rounded-[32px] md:rounded-[40px] border border-white/10 bg-slate-950/60 shadow-[0_0_80px_rgba(0,204,255,0.15)] backdrop-blur-2xl w-full mx-auto box-border">
              <div className="flex items-center gap-2 bg-black/40 py-1.5 px-4 rounded-full border border-white/10 shadow-inner">
                ...
                <span className="text-[10px] md:text-xs tracking-[0.25em] font-bold text-[var(--c)] uppercase" style={{ fontFamily: 'var(--fd)' }}> System Update </span>
              </div>
              <h2 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-black uppercase tracking-tighter leading-tight text-white my-2" style={{ fontFamily: 'var(--fd)', textShadow: 'var(--hero-text-shadow)' }}> WE WILL BE BACK<br /> <span className="text-[var(--c)]" style={{ textShadow: 'var(--king-text-shadow)' }}>SOON</span>
              </h2>
              <p className="text-xs sm:text-sm md:text-base text-gray-400 font-medium tracking-wide max-w-lg leading-relaxed px-2"> Our team is currently working to <span className="text-white">Checkmate this gambit</span>. </p>
              <div className="w-full h-px bg-gradient-to-r from-transparent via-white/10 to-transparent my-2 md:my-4" />
              <GlowButton parallelogram variant="brand" size="lg" onClick={onClose} className="min-w-full sm:min-w-[200px] shrink-0"> RETURN TO LOBBY </GlowButton>
            </motion.div>
          </div>
        </motion.div>
      )}

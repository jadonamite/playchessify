...
export default function GameHeader({
  ...
  return (
    <div className="flex flex-col md:flex-row md:items-center justify-between gap-2 md:gap-6 mb-3 md:mb-12">
      <div>
        <h1
          className="text-2xl sm:text-4xl md:text-5xl font-black uppercase tracking-tighter"
          style={{ fontFamily: 'var(--fd)' }}
        >
          {isBotGame ? (
            <>AI <span style={{ color: 'var(--c)' }}>Training</span></>
          ) : (
            <>Match <span style={{ color: 'var(--c)' }}>#{gameId}</span></>
          )}
        </h1>
        {renderGameInfo(
          isBotGame,
      ...
      <Link href="/app/lobby" className="hidden md:block">
        <GlowButton variant="ghost" size="sm" parallelogram>
          ← BACK TO LOBBY
        </GlowButton>
      </Link>
    </div>

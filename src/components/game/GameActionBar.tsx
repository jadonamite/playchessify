...
const getQuitButtonStyles = (holding: boolean, quitForfeits: boolean, danger: string) => ({
  flex: 1,
  height: 58,
  position: 'relative',
  overflow: 'hidden',
...
const getHintButtonStyles = (hintOff: boolean, isHintLoading: boolean) => ({
  flex: '0 0 auto',
  display: 'flex',
  alignItems: 'center',
  ...
  color: '#3a2400',
  boxShadow: hintOff ? 'none' : '0 6px 0 #b9791f, 0 10px 22px rgba(245,158,46,.35)',
...
const getQuitButtonText = (holding: boolean, quitForfeits: boolean) =>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, width: '100%', maxWidth: 660 }}>
        ...
        {gameOver ? (
          ...
          <motion.button
            ...
            style={getQuitButtonStyles(holding, quitForfeits, danger)}
          >
            ...
            <span className="relative" style={{ zIndex: 2, color: holding ? '#fff' : danger, ...LABEL }}>
              {getQuitButtonText(holding, quitForfeits)}
            </span>
          </motion.button>
        )}
        ...
        <motion.button
          ...
          style={getHintButtonStyles(hintOff, isHintLoading)}
        >

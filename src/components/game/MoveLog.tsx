...
const MoveHistoryItem = ({ san, index }: { san: string; index: number }) => (
  <div key={index} className={`flex items-center gap-2 text-xs font-mono p-1.5 rounded-lg ${index % 2 === 0 ? 'bg-white/5' : ''}`}> 
    <span className="text-[var(--t3)] w-5 text-left shrink-0">{Math.floor(index / 2) + 1}.</span> 
    <span className={index % 2 === 0 ? 'text-[var(--t1)]' : 'text-[var(--t2)]'}>{san}</span> 
  </div>
)
    ...
    <ClayCard variant="inset" className="p-5">
      ...
      <div className="grid grid-cols-2 gap-x-4 gap-y-1 max-h-[180px] overflow-y-auto pr-1 custom-scrollbar">
        {moveHistory.map((san, i) => (
          <MoveHistoryItem key={i} san={san} index={i} />
        ))}

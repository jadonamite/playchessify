...
export async function PATCH(req: NextRequest, { params }: Ctx) {
  ...
  if (!(await verifyMessage({
    ...
    message: `Chessify Profile Update\n\nAddress: ${address.toLowerCase()}\nTimestamp: ${timestamp}`,
  ...
  if (!(await checkRateLimit(address, 'update', 5, 3600))) {
    return NextResponse.json({ error: 'rate limit exceeded' }, { status: 429 });
  }

  if (username && !(await validateUsername(username)).ok) {
    return NextResponse.json({ error: (await validateUsername(username)).reason }, { status: 400 });
  }

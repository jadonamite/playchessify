...
import { getOnchainGameCached, verifyWalletSignature, GameStatus, type Address, } from '@/lib/celo-server';
...
export async function POST(
  ...
  if (!san || san.length > 16) return NextResponse.json({ error: 'invalid san' }, { status: 400 });
  if (!player || !/^0x[a-fA-F0-9]{40}$/.test(player)) return NextResponse.json({ error: 'invalid player' }, { status: 400 });
  if (!Number.isInteger(moveNumber) || moveNumber <= 0) return NextResponse.json({ error: 'invalid moveNumber' }, { status: 400 });
  ...
  try {
    ...
    if (existing.length >= moveNumber) return NextResponse.json({ error: 'move number already recorded', moves: existing }, { status: 409 });
    ...
    if (game.status !== GameStatus.Active) return NextResponse.json({ error: 'game not active' }, { status: 409 });
    ...
    if (player.toLowerCase() !== sideToMove.toLowerCase()) return NextResponse.json({ error: 'not your turn' }, { status: 403 });

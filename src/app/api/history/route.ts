...
const processGame = (game: any, id: string, me: string): HistoryItem | null => { 
...
export async function GET(req: NextRequest) { 
  ...
  try { 
    ...
    for (let i = 0; i < results.length; i++) { 
      ...
      const game = r.result as { white: string; black: string; wager: bigint; status: number | bigint; result: number | bigint; createdAt: number | bigint } 
      const item = processGame(game, ids[i], me) 

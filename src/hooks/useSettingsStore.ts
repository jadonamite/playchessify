...
const createSettingUpdater = (set: (state: Partial<SettingsState>) => void) => (
  <K extends keyof Omit<SettingsState, 'setSoundEnabled' | 'setBoardTheme' | 'setPieceSet' | 'setAiDifficulty' | 'setShowMoveHints'>>(
    key: K,
    value: SettingsState[K]
  ) => set({ [key]: value }) as (value: SettingsState[K]) => void
)
...
export const useSettingsStore = create<SettingsState>()(
  persist(
    (set) => ({
      ...
      aiDifficulty: 'medium',
      showMoveHints: true,
      setSoundEnabled: createSettingUpdater(set),
      setBoardTheme: createSettingUpdater(set),
      setPieceSet: createSettingUpdater(set),
      setAiDifficulty: createSettingUpdater(set),
      setShowMoveHints: createSettingUpdater(set),
    }),

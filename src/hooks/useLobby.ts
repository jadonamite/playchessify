    ...
    try {
      ...
      const result = await Promise.all(
        Array.from({ length: start - end + 1 }, (_, i) => start - i)
          ...
          .filter((g) => g && Number(g.status) === 0 && nowSecs - Number(g.createdAt ?? 0) <= JOIN_WINDOW_SECS && g.white !== '0x0000000000000000000000000000000000000000000')
          .map((g) => ({
            id: start - end + 1 - games.indexOf(g),
            creator: g.white,

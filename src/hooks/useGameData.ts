...
const deriveIdentity = (
  ...
  const myColor: 'white' | 'black' | null = isBotGame ? 'white' : isCreator ? 'white' : isOpponent ? 'black' : null;
  return { me, isCreator, isOpponent, isParticipant, myColor };
};

const deriveStatusFlags = (gameData: GameData | null) => {
  const drawProposer = norm(gameData?.drawProposer ?? '');
  const drawPending = drawProposer !== '' && drawProposer !== ZERO;
  const iProposedDraw = drawPending && drawProposer === norm(gameData?.white ?? '');
...
const deriveAdditionalData = (
  gameData: GameData | null,
  isBotGame: boolean,
  celoAddress: string | null | undefined,
  isConnected: boolean
) => {
  const canJoinFromPage =
    gameData?.status === '0' && !deriveIdentity(gameData, celoAddress, isBotGame).isParticipant && !isBotGame && isConnected;
  const wagerFormatted = gameData
    ? (Number(gameData.wager) / Math.pow(10, TOKEN_DECIMALS)).toFixed(0)
    : '0';
  const statusLabel = gameData
    ? STATUS_LABELS[gameData.status] ?? 'UNKNOWN'
    : '';
  return { canJoinFromPage, wagerFormatted, statusLabel };
};
...
export function useGameData({
  ...
  const identity = deriveIdentity(gameData, celoAddress, isBotGame);
  const statusFlags = deriveStatusFlags(gameData);
  const additionalData = deriveAdditionalData(gameData, isBotGame, celoAddress, isConnected);

...
const deriveIdentityAndStatus = (
  ...
  const canJoinFromPage = gameIsWaiting && !isParticipant && !isBotGame;
...
export function useGameData({
  ...
  const {
    ...
    payoutSettled,
    canJoinFromPage,
  } = deriveIdentityAndStatus(gameData, celoAddress, isBotGame);
  ...
  const playerAddrs = gameData
    ? [gameData.white, gameData.black].filter(
        (a) => a && a !== ZERO && a.startsWith('0x')
      )
    : [];
  ...
  return {
    ...
    contractDraw,
    payoutSettled,
    canJoinFromPage: canJoinFromPage && isConnected,
    wagerFormatted,
    statusLabel,
    gameProfileMap,
  };

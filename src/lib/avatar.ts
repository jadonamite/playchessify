...
function deriveColors(address: string): { c1: string, c2: string } {
...
function getPieceAndId(address: string): { piece: string, id: string } {
  const addr = address.toLowerCase()
...
export function generateAvatarSvg(address: string, size = 100): string {
  ...
  const { piece, id } = getPieceAndId(address)

...
export function getCaptureSummary(board: ReturnType<Chess['board']>): CaptureSummary {
  const { whiteCaptured, blackCaptured, whiteMaterial, blackMaterial } = calculateCaptureSummary(board)
  return {
    whiteCaptured,
    blackCaptured,
...
function calculateCaptureSummary(board: ReturnType<Chess['board']>) {
  const remaining = {

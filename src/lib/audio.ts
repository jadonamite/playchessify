...
function createMoveSoundSource(ctx: AudioContext, isOpponent: boolean) {
...
function createMoveSoundSub(ctx: AudioContext, isOpponent: boolean) {
  const t = ctx.currentTime
...
export function playMoveSound(ctx: AudioContext, isOpponent = false) {
  ...
  const { ns, ng } = createMoveSoundSource(ctx, isOpponent)
  const { sub, sg } = createMoveSoundSub(ctx, isOpponent)
  const t = ctx.currentTime
  ns.start(t)
  ns.stop(t + 0.14)
  sub.start(t)
